...
/*   Updated: 2025/10/17 15:54:48 by zait-err         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	cast_ray_textured(t_game *game, float ray_angle)
{
	...
	{
		ray.stepX = -1;
		ray.sideDistX = (game->player.x / TILE_SIZE - mapX) * ray.deltaDistX;
	}
	else
	{
		ray.stepX = 1;
		ray.sideDistX = (mapX + 1.0 - game->player.x / TILE_SIZE) * ray.deltaDistX;
	}

	if (ray.rayDY < 0)
	{
		ray.stepY = -1;
		ray.sideDistY = (game->player.y / TILE_SIZE - mapY) * ray.deltaDistY;
	}
	else
	{
		ray.stepY = 1;
		ray.sideDistY = (mapY + 1.0 - game->player.y / TILE_SIZE) * ray.deltaDistY;
	}
	...
	float wallX;
	if (side == 0)
		wallX = (game->player.y / TILE_SIZE + perpWallDist * ray.rayDY)
			- floor(game->player.y / TILE_SIZE + perpWallDist * ray.rayDY);
	else
		wallX = (game->player.x / TILE_SIZE + perpWallDist * ray.rayDX)
			- floor(game->player.x / TILE_SIZE + perpWallDist * ray.rayDX);
